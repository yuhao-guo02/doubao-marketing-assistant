...
def get_channel_prompts(product_info, channel_type):
    ...
    if channel_type == 'poster':
        # 海报大图提示词
        discount_info = f"{discount_rate}%折扣" if discount_rate else ""
        image_prompt = f"""请生成一张竖版海报图片，需完整呈现以下产品信息：{product_name}、核心卖点（{product_features}）、价格（{price_range}）、折扣（{discount_info}）。图片风格需贴合 {product_category} 的行业特性，色彩搭配鲜明且主次分明，文字布局清晰易读，重点信息（价格、优惠）需用加粗/亮色突出。可适当添加场景化元素增强吸引力，避免信息堆砌感。"""
        ...
        # Banner提示词
        discount_info = f"{discount_rate}%折扣" if discount_rate else ""
        image_prompt = f"""请生成一张横版图片，画面以 {product_name} 为视觉焦点。图片中的文案需极度精简，仅保留 2-3 个关键信息：{product_name}、{discount_info}、行动指令（如 "立即抢购"）。文字字体需醒目但不遮挡画面主体，可搭配简约符号（如箭头、折扣标签）强化引导。整体风格色彩明快，适合在首页/频道页顶部展示，能快速抓住用户注意力，避免信息堆砌感。"""
        ...
        discount_info = f"，{discount_rate}%折扣" if discount_rate else ""
        text_prompt = f"""请为 {product_name} 生成一段配套文案（不超过 30 字），需包含产品名称和 1 个核心卖点（如 "{product_name}，{product_features}{discount_info}"），语言口语化、无修饰，适合在美团信息流中自然展示，给人轻松种草的感觉。"""
...
if __name__ == '__main__':
    ...
    # 获取端口，Railway会自动设置PORT环境变量
    port = int(os.environ.get('PORT', 8080))
    
    print("豆包AI Railway部署应用启动中...")
